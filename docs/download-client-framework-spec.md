...
| 9 | ~~**Action call shapes and the `{"status": …}` contract**~~ **PARTLY FALSIFIED, 2026-08-22, measured against SAB 5.1.1 ([#23](https://github.com/crzykidd/lftpweb/issues/23))** — an **authentication** failure is not a `{"status": false}` JSON envelope at all: it is **HTTP 403, `text/html`, body `API Key Incorrect`**. `response.json()` raises on it, so every authenticated call (`list_transfers`, `list_history`, `list_base_paths`) currently reports "non-JSON body"/"HTTP 403" instead of "wrong API key". The `{"status": …}` contract for *non-auth* action failures remains unverified | Still the connector's riskiest area, now for a measured reason rather than a suspected one |
