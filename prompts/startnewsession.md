...
**Practice since 2026-08-21, at the user's request: open work is tracked in GitHub issues, not only
in `prompts/`.** A written handoff prompt is linked from its issue. **Read the issues before assuming
something is untracked**, and open one for anything deferred rather than letting it live only in a
session transcript.

**Only three are open.** #14/#15/#16/#17/#19 all shipped in v0.3.1 and were closed 2026-08-22.
...
**PR #20** (`dev` → `main`, merged `03ad151`), tag **`v0.3.1`**, release notes = the `[0.3.1]`
CHANGELOG section verbatim (10,742 chars). `:latest`/`:0.3.1`/`:0` published on the release event.
**A clean cut, second in a row** — CI, image publish, both CodeQL analyses and registry retention all
green, no bypasses, no re-runs. Tests at release: **1685 backend / 680 frontend, 0 skipped**, with
both fake seedboxes up so the integration suite genuinely ran. **Migration 026** (`metric_daily`)
runs on first start; additive.

`0.3.1` was the user's explicit choice over `0.4.0` despite six features and a migration — same
sequential-numbering preference as at `0.3.0`. The changelog archive step stays deferred.

What shipped, all of it user-verified in a browser first:

| What | Commit |
|---|---|
| Pause for a fixed duration — **one dropdown** (*Till I unpause / 1 / 10 / 30 / 60 min*, selection acts immediately) plus a **"Pause after active"** checkbox, disabled when nothing is running. Deadline is a stored absolute `paused_until`, not a timer; expiry evaluated in `TransferQueue.tick()` **and** synchronously in `start()`, so an app down past the deadline comes back unpaused | `1791af8`, `3539990` |
| Paused/retried queued rows show their own partial-bytes progress (`QUEUED n%`, indigo fill — **user-confirmed on screen**). Needed a backend projection: a **fresh retry inserts a new `job` row whose `bytes_done` starts at 0**, so `_job_out` prefers `item.local_size` for `queued` rows | `32cf5fd` |
| **Bandwidth is now a ceiling plus a throttle.** Settings → Transfer owns `max_bandwidth_bps` (the ceiling); the Queue slider owns `throttle_bandwidth_bps` below it. `effective_bandwidth_bps()` feeds `SchedulerSettings.max_bandwidth_bps` **at the call site** — `core/scheduler.py` and §4.5's worked examples untouched. Clamping is on **write** (a read-side clamp would let a lowered-then-raised ceiling silently restore the old throttle). The fast-lane reserve derives from the *effective* limit, or a 100 MB/s ceiling throttled to 1 MB/s would reserve 10 MB/s against a 1 MB/s budget and admit nothing, ever | `764aaa7`, `21f4a5f` |
| The slider **commits itself**: a checkbox "Apply to new items only" (checked) replaces the two-button fork, commit fires 5 s after the last change with a countdown line that becomes the result banner, and re-dragging re-arms it (so dragging back cancels). The paused case reports **"nothing was restarted"** rather than a restart that did not happen | `21f4a5f` |
| Daily per-queue metric rollups + "Total downloaded" + 90d/1y. **Rollup runs before prune in one function** — the only data-destroying failure mode here, asserted by a test that seeds a day old enough to be pruned by the same `run_once` and proves the daily row survived. Raw retention default 7 → 30 days | `8ae1e53` |
| Dashboard chart **range and grouping decoupled** (hour/day/week/month), 7d moves 6h→daily, 90d/1y default weekly. **Hourly at 90d/1y is impossible** (raw retention 30 d, `metric_daily` is one-day) and is rejected server-side with a 422, never silently coarsened. Week/month are summed from daily rows — no new table | `1bb398e` |
| "Rescan now" on the Queue tab, sharing a new `useRescan` hook with the Files page | `0a0c48b` |
| *arr poll cadence default 60 s → 10 s, exposed as **"How often to check Sonarr/Radarr"** on Settings → Integrations. The issue's "12× the request rate" premise was **wrong**: the queue is one request per instance per pass, history is already by-`downloadId` and event-triggered, and both symptoms gate on the queue poll. No cadence split, no adaptive logic | `60f174f`, `5a04265` |

**The browser-test round is recorded in `prompts/test-findings-2026-08-21.md`** — six findings, all
fixed in this release. Worth reading for the reasoning, not the status.

### 📸 Screenshots — still stale, and now the blocker on the README

Unchanged from v0.3.0 and **still the one actively-wrong thing on the front page**: README's second
hero is captioned "The Events page" while showing the *removed* History page. `docs/screenshot-plan.md`
has the shot list; its **v0.3.1 corrections block at the top is the part to read** — the old bandwidth
prep advice is wrong for the demo tree (use ~1 MB/s, not 10) and the pause-control notes describe a
menu that no longer exists.

Test data: **`uv run python docker/test-seedbox/make_demo_tree.py`** generates it with generic names.
Researched 2026-08-22 and written into the plan: the only real titles that survive scrutiny in a
public README are the **Blender open movies** (CC-BY, real TMDB entries) and **Sita Sings the Blues**
...
| U — what's-new popup, Docs → Release notes page, version link points in-app | User request, design settled 2026-08-17. New `lib/releaseNotes.ts` (pure, unit-tested against the real `CHANGELOG.md?raw`) parses the changelog into per-version sections and decides what the popup shows on the first page load after an upgrade — nothing on a fresh browser, an unchanged version, or a downgrade; every skipped release, newest first, otherwise — writing `whatsnew.lastSeenVersion` to `localStorage` (per-browser, named limitation, no server-side seen-state) silently either way. New `components/WhatsNewDialog.tsx` (mounted once in `Layout.tsx`, its own one-shot `getHealth()`, matching `VersionLink.tsx`'s pattern rather than `StatsHeader.tsx`'s poll) renders it. New Docs → Release notes page (`pages/docs/ReleaseNotesPage.tsx`, registered in `nav.ts`/`App.tsx`) renders `CHANGELOG.md` **verbatim** — not through `MarkdownDoc`'s structural `parseDocSource` (which throws on the changelog's intro paragraphs + commented-out skeleton), but through its newly-exported `SectionBody` fed the whole raw file as one blob; gained a "View on GitHub" line from its own one-shot health read. `lib/versionBadge.ts`'s non-dev branch now links to the in-app `/docs/release-notes` route instead of the GitHub release tag (dev-channel commit link unchanged); `VersionLink.tsx` renders a router `Link` for that case via `lib/docLinks.ts`'s existing `classifyLink`, never a full page reload. `docker/Dockerfile`'s frontend-builder stage copies `CHANGELOG.md` in alongside the existing `docs/` copy; no `vite.config.ts` change needed (`fs.allow: ['..']` already covers the repo root) — confirmed with both `npm run build` and a real `docker build --target frontend-builder`. Full reasoning, including the "third independent `getHealth()` call, not a shared context" trade-off, in `docs/decisions.md` (2026-08-17). **Unviewed — no browser in this environment.** | ✅ done, `prompts/done/2026-08-17-whats-new-popup-and-release-notes.md` |
| V — orphaned spent-archive rows self-heal with their parent | Live production defect, fully diagnosed from logs: a rar'd release ran the whole pipeline correctly (verify, extract, `delete-archives-after-extract` removed the spent volumes, *arr import, remote copy deleted, *arr cleanup removed the local directory) and the parent row left the Files page via `REMOVED_BOTH` — but its archive-volume child rows stayed behind forever, resting `EXCLUDED` with no parent and no delete affordance, because `core/engine.py._persist`'s `deleted_archive_paths` branch resolved unconditionally, every pass, with no notion that the parent itself could also leave both trees. Fix: that branch now checks whether the row's own top-level ancestor is still in `written` (this pass's "present or still publishable" set) before resting at `EXCLUDED` — an ancestor mid-grace reads present too, since a non-terminal vanished resolution re-enters `written` before any of its children are visited in the same pass (`sorted(vanished)` guarantees ancestor-before-descendant). Once the ancestor is confirmed gone, the row falls straight to `REMOVED_BOTH` and a new `core/archive_cleanup.py.purge_deleted_archive_paths` (re-exported from `core/local_delete.py`) clears its `deleted_archive` registry entry. Self-heals any already-orphaned row within a scan pass or two, no migration. Full reasoning, including the top-level-file defensive carve-out that protects the existing (2026-08-14) test suite, in `docs/decisions.md` (2026-08-17). | ✅ done, `prompts/done/2026-08-17-orphaned-spent-archive-rows.md` |
| W — stranded source delete completes the move-delete ladder's error path, plus two notify-trust gaps closed the same day | Live production/test defect, `arr_imported` → a failed rung-4 deferred source delete (`SSH connection closed`) → `arr_cleanup` ran anyway seconds later, permanently stranding the remote copy with no Delete affordance in the UI. Fix, three parts: (1) `core/arrsync.py._sweep_stranded_source_deletes` retries every pass, keyed off `item.remote_delete_pending` itself rather than the one-shot `imported` transition — self-healing any row already stranded before this shipped, no migration — with `_InstanceBackoff`-shaped backoff bounded at `MAX_SOURCE_DELETE_RETRY_ATTEMPTS` (one `remote_delete_retries_paused` event, not a `remote_delete_failed` every pass while a seedbox is down); `_maybe_cleanup` now withholds while the debt is owed, restoring ladder order as enforced rather than hoped-for; `_commit_terminal`'s `gone` branch now names a still-pending source delete in its own event (visibility only). (2) `lib/fileTree.ts.canDeleteLocal` widened (moved from a private `FileTree.tsx` function, pure + tested) to offer Delete for local content **or** a surviving remote copy, not only local — the manual Source-scope escape hatch was unreachable for exactly the rows this retry's own failure mode produces; new `shouldOfferLocalScope` symmetrically gates the dialog's Local checkbox. **Two same-day scope additions, folded in** (both closing the other half of the same incident — production *arrs mounting the synced storage at a different container path than lftpweb, so every notify push landed nowhere real): `_maybe_warn_path_mismatch` (predictive) compares a matched queue record's own `outputPath` against what a notify would push and writes one debounced `arr_path_mismatch` warning per `(queue, derived root)` naming the `arr_visible_path` fix, the moment a match commits — before the first notify ever fires; and scan-command outcome verification (confirmed) — `core/arrnotify.py.notify_arr` now persists the pushed command's own id (`item.arr_scan_command_id`, migration 021, a column not memory since two different processes' objects call `notify_arr`), and `core/arrsync.py._check_scan_commands` polls `GET /api/v3/command/{id}` on later passes, writing `arr_scan_command_failed` on a confirmed failure, clearing silently on `completed`/404/a bounded check-count giving up. `DESIGN.md` §7.3/§7.4/§16 updated; full reasoning (three separate decisions, all 2026-08-17) in `docs/decisions.md`. **Unviewed — no browser in this environment.** | ✅ done, `prompts/done/2026-08-17-stranded-source-delete-retry.md` |
| X — user request, Logs text filter + deeper lookback (row lettered X — a row W already existed for the same day) | User request, design settled 2026-08-17: Settings → Logs needed a search/filter over the displayed raw log lines, plus a deeper lookback than the old 2,000-line ceiling — the *arr integration's per-minute poller HTTP lines now dominate a busy install's log, so 2,000 lines covered well under an hour. `core/logtail.py.MAX_LINES_CAP` raised 2,000 → 10,000; `DEFAULT_MAX_BYTES` (the per-tail byte ceiling) raised 2 MB → 5 MB, mirroring `logsetup.MAX_BYTES` via a code comment rather than an import (layering: `core/` can't reach up into process-wide logging setup) — 10k lines at typical line lengths comfortably fits in 5 MB. Frontend: `Lines` options extended to `[100, 200, 500, 1000, 2000, 5000, 10000]`; a new text-filter input does a case-insensitive, client-side, instant substring match over the *fetched window only* — deliberately not a server-side grep across rotated files, since at 10k lines the fetched window can already span an entire live file, which is the point of pairing the two features (`docs/decisions.md`) — with a "showing N of M lines" readout while active, no match highlighting in v1. New pure `lib/logFilter.ts` (`filterLogLines`/`logFilterSummary`), Vitest-covered; `LogsTab.tsx` stays thin. The lines view was already one `<pre>` block (a single DOM text node, not 10k row elements) rather than per-row divs, so it was left un-virtualized — `HistoryEventsSection.tsx`'s virtualizer solves a different problem (many discrete DOM rows) that this view doesn't have. **Unviewed — no browser in this environment.** | ✅ done, `prompts/done/2026-08-17-logs-search-and-lookback.md` |
| Y — user request, Settings → Logs gains a "Support bundle" download | User request, design settled 2026-08-17: a "Support bundle…" button on Settings → Logs opens a checkbox dialog (all default ON) and produces one downloadable **zip** via a new `POST /api/support-bundle` — the user said "rar"; settled as zip (RAR creation is proprietary, the image has no `rar` binary, `zipfile` needs no new dependency, `docs/decisions.md`). New `core/supportbundle.py` (framework-free: zip building, per-*arr-instance log fetch over two new `core/arrclient.py.ArrClient` methods — `log_files`/`download_log_file`, the latter streamed and capped at a 20 MB per-instance budget — disk usage, `lftp`/Python version reads) + `api/support_bundle.py` (the thin assembly layer, added to `tests/test_auth_api.py.PROTECTED_ROUTE_TEMPLATES`). Bundle contents: lftpweb's own rotated logs (always included, checkbox checked+disabled); `bundle/environment.json` (version, build, migration level, the `/api/health` payload, `lftp`/Python versions, per-queue disk usage via `shutil.disk_usage`); `bundle/settings.json` — built by calling the *same* row→response-model functions the settings endpoints already return (`_host_out_from_row`, `_queue_out_from_row`, `_instance_out_from_row`, `_pattern_out_from_row`, `_postprocess_out`, `api/jobs.py`'s transfer-settings equivalent) rather than hand-picking columns, so a secret can only ever leak here if it already leaks through the authenticated Settings API itself; `bundle/events.ndjson` (most recent 1,000, reusing `api/history.py`'s own `_events_where_clause`/`_event_out`); `bundle/jobs.ndjson` (most recent 100, including `output_tail` — deliberately *not* routed through `HistoryJobOut`, which excludes it by design for the paginated History list); one `bundle/arr-<name>/` directory per selected, still-enabled *arr instance. **Deliberately excluded:** the SQLite database itself (carries every encrypted secret), `known_hosts` pins, the install secret; no redaction pass on fetched *arr logs (the user's own opt-in per instance). A per-instance *arr fetch failure (unreachable, bad key, undecryptable stored key) writes a `FETCH-FAILED.txt` marker in that instance's own directory rather than failing the whole bundle; every *arr-supplied name (instance name, log filename) is floored to a single safe zip-path component (`_safe_zip_component`) since `zipfile.ZipFile.writestr` doesn't sanitize its target path the way `extractall` does. One `support_bundle_created` info event is written per bundle, naming the selected parts. Frontend: new pure `lib/supportBundle.ts` (`defaultSupportBundleSelection`/`enabledArrInstances`/`toggleArrInstance`/`toSupportBundleRequest`, Vitest-covered) + `components/SupportBundleDialog.tsx`, wired into `LogsTab.tsx`; `api/client.ts.downloadSupportBundle` fetches the zip as a blob and synthesizes the download (the log/backup GETs' plain `<a href download>` has no POST-body equivalent), filename taken from the response's own `Content-Disposition` header. `docs/concepts.md` gained a "What's in a support bundle" section. Tests: `tests/test_support_bundle_api.py` (selection-exactness, the secrets-absence assertion against a seeded real password + *arr API key, *arr fetch-failure-is-a-marker-not-a-500, the audit event) plus `tests/fake_arr.py` extended with the two log-file endpoints. **Unviewed — no browser in this environment.** | ✅ done, `prompts/done/2026-08-17-support-bundle.md` |
| Z — support-bundle polish, four flaws from the first real bundle | The user's first real support bundle (test system, 2026-08-17) surfaced four flaws row Y's fake-*arr fixture couldn't have caught, plus a README ask. (1) `bundle/settings.json` was exporting `postprocess.extract_passwords` verbatim — user secrets like any other; `api/support_bundle.py._gather_settings` now swaps the list for `extract_passwords_count` on the bundle's own copy of that dict, after calling the shared `_postprocess_out` conversion (the real `/api/settings/postprocess` response is untouched). (2) The ~20 MB *arr log budget was written to read "per instance" but was applied per file — one real Sonarr with 53 debug files produced a 54 MB uncompressed folder. `core/supportbundle.fetch_arr_instance_logs` now keeps a running total across the instance's whole file list (checked before each fetch, `ARR_LOG_BYTE_BUDGET`), fetches newest-first (`_log_file_sort_key`: non-rotated names first, then rotations in ascending — i.e. newest-first — numeric order), and writes a `TRUNCATED.txt` naming what didn't fit once the budget's gone; `ARR_LOG_PER_FILE_BYTE_CAP` (same value) remains a secondary per-download ceiling. (3) One file's fetch failing (observed live: a 404 on a custom-script log the *arr's listing names but serves from a different endpoint) no longer produces the same `FETCH-FAILED.txt` an unreachable instance gets — that marker is now instance-level only (unreachable, bad/undecryptable key, the listing request itself failing); a per-file failure writes `<filename>.FETCH-ERROR.txt` beside the files that did fetch and the loop continues. `tests/fake_arr.py` gained `FakeArrState.broken_log_files` (listed but always-404 filenames) to model this without touching the existing fixtures. (4) `bundle/settings.json` gained the one `*Settings` group it was missing — backup (`load_backup_settings` + `BackupSettingsOut`, `api/backup.py`'s own GET conversion inlined, since that module has no factored `_backup_out` helper the other settings routers do). README gained a "Support bundle" section (captures/never-captures/the *arr-logs-unredacted caveat/where the button lives) pointing at `docs/concepts.md#support-bundle` for the full per-part table rather than duplicating it; that doc's own section updated for all four fixes. Backend-only — frontend untouched, re-verified anyway. Full reasoning in `docs/decisions.md` (2026-08-17). | ✅ done, `prompts/done/2026-08-17-support-bundle-polish.md` |
